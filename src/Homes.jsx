...
class Homes extends React.Component {
  ...
  title = "LAND ON YOUR FAVOURITE JOB";
  ...
  async getRequest(url) {
    ...
    try {
      ...
      alert(
        "The request failed. Perhaps the server is not running, or the Jobs API might be down."
      );
      json = [];
    }
    return json;
  ...
  buildUrl(params, page) {
    return encodeURI(
      `${this.baseUrl}?description=${params.description}&location=${params.location}&full_time=${params.full_time}&page=${page}`
    );
  }
  ...
  async checkNextPage(response, params, currentPage) {
    ...
    if (response.length < 50) {
      ...
      const nextPage = await this.getRequest(
        this.buildUrl(params, currentPage + 1)
      );
